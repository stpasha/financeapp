...
public class OutboxScheduler {
    ...
    public void process() {
        for (UUID id : outboxEventRepository.findOutboxEventIdByPendingStatus(1000)) {
    ...
    public void processErrorEvents() {
        for (UUID id : outboxEventRepository.findRetryableOutboxEvent(LocalDateTime.now(), 1000)) {
            eventProcessor.processSingleEvent(id);
        }
    }
}
